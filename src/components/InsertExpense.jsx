...
export default function InsertIncome() {
    ...
    const { insertExpense, setInsertExpense } = useStateContext()
    ...
    return (
        <div className={`fixed h-screen w-screen top-0 left-0 flex items-center justify-center
            bg-black bg-opacity-50 transform transition-transform duration-400 ${insertExpense}`}
        >
            <div className="bg-white shadow-xl shadow-black rounded-xl w-10/12 md:w-2/5 h-7/12 p-6">
                <form className="flex flex-col" onSubmit={handleSubmit}>
                    <div className="flex justify-between pb-8 items-center">
                        <p className="font-bold">Insert Expense Details</p>
                        <button
                            type="button"
                            onClick={closeModal} 
                    ...
                    <button 
                        className="inline-block px-6 py-2.5 my-3 bg-green-500 font-md text-md leading-tight rounded-full shadow-md hover:bg-green-700"
                        type="submit"
                    >
                        Submit 
                    </button>
